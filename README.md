...
This repository contains the ontological structure and the knowledge graph used in the [HerMeS project](https://hermes.cnr.it/progetto/), an initiative by the CNR and DigiLab Sapienza aimed at managing and enhancing cultural heritage through semantic technologies.
...
Please note that the repository contains previous versions of the ontology and the knowledge graph (v1.0). These versions are retained to track changes and revisions during ontology development. 

For usage, please always refer to the latest version published within this repository.
...
To interact with an instance of the knowledge graph, it is possible to set up a SPARQL Endpoint using the framework [Apache Fuseki](https://jena.apache.org/documentation/fuseki2/). In addition to the official documentation of the framework, a Docker image is available that can be easily configured to expose Fuseki services thorugh an intuitive web application.
...
When Docker is correctly running on your machine, download the latest ```stain/jena-fuseki``` image from the [Docker Hub](https://hub.docker.com/r/stain/jena-fuseki)
...
The command above creates a container with ```--name``` fuseki on the host machine. The container is created from the image ```stain/jena-fuseki``` and is reachable through the port ```3030``` of the host machine. Specifically, the Docker command maps the host port ```3030``` to the container port ```3030``` where the Fuseki endpoint actually runs. 
...
The access to the SPARQL endpoint requires admin authentication; the parameter ```-e ADMIN_PASSWORD=<PWD>``` allows specifying the password to use. Replace ```<PWD>``` with the password you intend to use for authentication.

Now the container is running, and you can access the endpoint at ```http://localhost:3030``` using your browser.

![Figure 3. Homepage of the Fuseki SPARQL endpoint running on the host machine on port 3030](assets/readme/Fuseki_homepage.png)


To create a new dataset, go to the page 'manage' and click on 'new dataset' by specyfing a name, e.g., 'HERMES-KG_v1.1'. At this point, the page 'manage' should look like the picture below.

![Figure 4. Management page  of the Fuseki SPARQL endpoint](assets/readme/Fuseki_manage.png)
...
![Figure 4. Query page of the Fuseki SPARQL endpoint](assets/readme/Fuseki_query.png)
